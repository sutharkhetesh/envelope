...
function printEnvelope(address: Address) {
  ...
  const cityLine = [address.city, address.state, address.postal_code, address.country]
  ...
  const ghostLines = LINES_Y.map(
    y => `<div style="position:absolute;left:${LINES_X}in;top:${y}in;width:${LINES_W}in;border-bottom:1px solid #b0bcc8;"></div>`
  ).join('');
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(160deg,#eef2ff 0%,#f8fafc 60%)' }}>
      ...
      {/* ── Header ── */}
      <header style={{ background: 'linear-gradient(135deg,#1e3a5f 0%,#2563eb 100%)' }}
        className="shadow-lg">
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h1 className="text-xl font-bold text-white leading-tight tracking-tight">Envelope Printer</h1>
              <p className="text-blue-200 text-xs mt-0.5">10&Prime; &times; 4.5&Prime; &nbsp;|&nbsp; Address Manager &amp; Print</p>
            </div>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-5">
          ...
          /* ── Empty state ── */
            ...
            <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
              ...
              <p className="text-sm text-gray-400 mb-5">
                {search ? 'Try a different search term' : 'Add your first address to get started'}
              </p>
              {!search && (
                <button onClick={openAdd}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    ...
                    <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider hidden md:table-cell">Company</th>
                    <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Address</th>
                    <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider hidden lg:table-cell">Phone</th>
                      ...
                      <tr key={addr.id} className="hover:bg-blue-50/40 transition-colors group">
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center justify-end gap-1">
                            {/* Print */}
                            <button onClick={() => printEnvelope(addr)} title="Print Envelope"
                            ...
                            {/* Delete */}
                            <button onClick={() => handleDelete(addr.id, fullName)} title="Delete"
        ...
        {!loading && addresses.length > 0 && (
          <p className="text-xs text-gray-400 text-right pr-1">
            Showing {addresses.length} {addresses.length === 1 ? 'address' : 'addresses'}
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div>
                <p className="text-xs font-bold text-blue-600 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-blue-100 inline-flex items-center justify-center text-blue-600 text-[10px] font-bold">2</span>
                  Company
                </p>
                <div>
                  <label className={label}>Company Name</label>
                  <input type="text" className={input} placeholder="Acme Corp (optional)"
              ...
              <div>
                <p className="text-xs font-bold text-blue-600 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-blue-100 inline-flex items-center justify-center text-blue-600 text-[10px] font-bold">3</span>
                  Contact
                </p>
                <div>
                  <label className={label}>Phone Number <span className="text-red-500 normal-case">*</span></label>
                  <div className="flex gap-2">
                    ...
                    {form.phone_country_code === 'other'
                      ? <input type="text" className={`${input} w-20 flex-shrink-0`} placeholder="+00"
                          value="" onChange={e => setForm(f => ({ ...f, phone_country_code: e.target.value }))}/>
                      : null}
                    <input type="tel" className={`${input} flex-1`} placeholder="9876543210"
              ...
              <div>
                <p className="text-xs font-bold text-blue-600 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-blue-100 inline-flex items-center justify-center text-blue-600 text-[10px] font-bold">4</span>
                  Address
                </p>
                <div className="space-y-3">
                  <div>
                    <label className={label}>Address Line 1 <span className="text-red-500 normal-case">*</span></label>
                    <input type="text" className={input} placeholder="House / Shop No., Street"
                      value={form.address_line1} onChange={set('address_line1')}/>
                  </div>
                  <div>
                    <label className={label}>Address Line 2</label>
                    <input type="text" className={input} placeholder="Area, Landmark (optional)"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="text" className={input} placeholder="Delhi"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="text" className={input} placeholder="110006"
